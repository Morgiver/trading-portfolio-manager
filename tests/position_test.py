...
class TestPosition(unittest.TestCase):
    def test_long_position_initialization(self):
        with self.assertRaises(Exception):
            Position('01/01/2000, 00:00:00', 1.0, 0.9, 1.1, 1.0, 100000.0, 0.01, BUY_SIDE, "forex", "Oanda", "EURUSD", ASSET_FUTURE)

    def test_short_position_initialization(self):
        with self.assertRaises(Exception):
            Position('01/01/2000, 00:00:00', 1.0, 1.1, 0.9, 1.0, 100000.0, 0.01, SELL_SIDE, "forex", "Oanda", "EURUSD", ASSET_FUTURE)

    def test_position_pip_size(self):
        p = Position('01/01/2000, 00:00:00', 1.0, 1.1, 0.9, 1.0, 100000.0, 0.01, BUY_SIDE, "forex", "Oanda", "EURUSD", ASSET_FUTURE)
    ...
    def test_get_pnl(self):
        long = Position('01/01/2000, 00:00:00', 1.0, 1.1, 0.9, 1.0, 100000.0, 0.01, BUY_SIDE, "forex", "Oanda", "EURUSD", ASSET_FUTURE)
        short = Position('01/01/2000, 00:00:00', 1.0, 0.9, 1.1, 1.0, 100000.0, 0.01, SELL_SIDE, "forex", "Oanda", "EURUSD", ASSET_FUTURE)
    ...
    def test_update_by_candle_raising(self):
        p = Position('01/01/2000, 00:00:00', 1.0, 1.1, 0.9, 1.0, 100000.0, 0.01, "toto", "forex", "Oanda", "EURUSD", ASSET_FUTURE)
        with self.assertRaises(Exception):
            p.update_by_candle({'Date': '01/01/2000, 00:00:01', 'High': 1.02, 'Low': 1.009, 'Close': 1.01})
